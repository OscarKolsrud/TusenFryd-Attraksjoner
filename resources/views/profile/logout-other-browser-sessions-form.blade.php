<x-jet-action-section>
    <x-slot name="title">
        {{ __('Nettleser Sesjoner') }}
    </x-slot>

    <x-slot name="description">
        {{ __('Administrer og logg ut dine aktive økter på andre nettlesere og enheter.') }}
    </x-slot>

    <x-slot name="content">
        <div class="max-w-xl text-sm text-gray-600">
            {{ __('Om nødvendig kan du logge av alle andre nettleserøkter på tvers av alle enhetene dine. Hvis du føler at kontoen din er kompromittert, bør du også oppdatere passordet ditt.') }}
        </div>
        ...
        <div class="flex items-center mt-5">
            <x-jet-button wire:click="confirmLogout" wire:loading.attr="disabled">
                {{ __('Logg ut andre nettlesersesjoner') }}
            </x-jet-button>
        ...
        <x-jet-dialog-modal wire:model="confirmingLogout">
            ...
            <x-slot name="content">
                {{ __('Vennligst skriv inn passordet ditt for å bekrefte at du vil logge av de andre nettleserøktene på alle enhetene dine.') }}
